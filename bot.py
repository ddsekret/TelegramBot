...
from parser import (
    parse_by_keywords,  # Основная функция для парсинга данных водителя
    parse_carrier_data,
    parse_customer_data,
    normalize_data,
    parse_phone_numbers,
...
class DatabaseManager:
    ...
    async def lookup_carrier(self, carrier_name):
        try:
            ws = self.carriers_wb.active
            # Проверяем по столбцу "Короткое название" (второй столбец, индекс 1)
            for row in ws.iter_rows(min_row=2, values_only=True):
    ...
    def ensure_files_exist(self):
        for file_type, path in self.files.items():
            if not os.path.exists(path):
                ...
                ws.append(headers)  # Исправлено: ws.append.headers) → ws.append(headers)
                wb.save(path)
...
async def process_add_driver(message: types.Message, state: FSMContext):
    try:
        ...
        # Используем parse_by_keywords вместо parse_driver_data
